...
namespace EvolutionEngine
{
	...
	{
		SDL_Helper::Instance()->SDL_DestroyTexture(m_loadingBG);
		delete(this->m_loadingText);
	}

	void LoadingScreen::Start()
	{
		this->m_loadingText = new Text(LocalizationManager::Instance()->GetText("loadingText"), 1000, 650, 25, true, FONT_NORMAL, C_WHITE);	// We wanna use roboto instead of custom font here
		SDL_Helper::Instance()->SDL_LoadImage(&this->m_loadingBG, IMG_BG_LOADING);
		SDL_Helper::Instance()->SDL_PauseMusic();
	}

	void LoadingScreen::Draw()
	{
		SDL_Helper::Instance()->SDL_DrawImage(this->m_loadingBG, 0, 0);
		this->m_loadingText->Draw();
	}
	...
	{
		SDL_Helper::Instance()->SDL_SetMusicVolume(MIX_MAX_VOLUME);
