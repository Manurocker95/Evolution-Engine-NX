...
namespace EvolutionEngine
{
	...
	Sprite::Sprite(int _x, int _y, char * _sprite, int _numFramesX, int _numFramesY, int _sizePerFrameX, int _sizePerFrameY, int _currentFrameX, int _currentFrameY, bool _multipleFrames, bool _animated, int _deltaTimeReduction, bool _drawOpacity, bool _draggable, int _ox, int _oy, bool _limitedInScreen)
	{
		this->m_opacity = 255;
		this->m_active = true;
		...
		SDL_Helper::Instance()->SDL_LoadImage(&this->m_sprite, _sprite);
	}

	Sprite::~Sprite()
	{
		//if (this->m_sprite != NULL)
		//	SDL_Helper::Instance()->SDL_DestroyTexture(this->m_sprite);
	}
		...
		if (this->m_active)
		{
			if (this->m_drawOpacity)
			{
				if (this->m_multipleFrames)
					SDL_Helper::Instance()->SDL_DrawImageRectOpacity(this->m_sprite, this->m_x, this->m_y, this->m_currentFrameX * this->m_sizePerFrameX + this->m_ox, this->m_currentFrameY * this->m_sizePerFrameY + this->m_oy, this->m_sizePerFrameX, this->m_sizePerFrameY, this->m_opacity);
				else
					SDL_Helper::Instance()->SDL_DrawImageOpacity(this->m_sprite, this->m_x, this->m_y, this->m_opacity);
			}
			else
			{
				if (this->m_multipleFrames)
					SDL_Helper::Instance()->SDL_DrawImageRect(this->m_sprite, this->m_x, this->m_y, this->m_currentFrameX * this->m_sizePerFrameX + this->m_ox, this->m_currentFrameY * this->m_sizePerFrameY + this->m_oy, this->m_sizePerFrameX, this->m_sizePerFrameY);
				else
					SDL_Helper::Instance()->SDL_DrawImage(this->m_sprite, this->m_x, this->m_y);
			}
		}
	}
	...
	void Sprite::LoadNewSprite(char * _img)
	{
		SDL_Helper::Instance()->SDL_DestroyTexture(this->m_sprite);
		SDL_Helper::Instance()->SDL_LoadImage(&this->m_sprite, _img);
	}
}