...
namespace EvolutionEngine
{
	...
	MusicSound::MusicSound(char * _file, bool _inLoop, int _channel) : Sound(SOUND_TYPE::MUSIC, _inLoop, _channel)
	{
		LoadAudio(_file);
	}
	...
	void MusicSound::LoadAudio(char * _file)
	{
		SDL_Helper::Instance()->SDL_LoadMusicAudio(&this->m_music, _file);
	}

	void MusicSound::End()
	{
		SDL_Helper::Instance()->SDL_PauseMusic();
		SDL_Helper::Instance()->SDL_DestroyMusicAudio(this->m_music);
		delete(this->m_music);
	}

	void MusicSound::Play()
	{
		SDL_Helper::Instance()->SDL_PlayMusicAudio(this->m_music, this->m_channel, this->m_inLoop);
	}

	void MusicSound::Resume()
	{
		SDL_Helper::Instance()->SDL_ResumeMusic();
	}
